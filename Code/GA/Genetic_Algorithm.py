...
def visualize_SUMO(chromosome=None,n_steps=n_steps):
    ...
    personwaiting_media = []
    vehiclewaiting_media = []
    for step in range(n_steps):
        personwaiting = []
        vehiclewaiting = []
        all_ids=traci.person.getIDList()+traci.vehicle.getIDList()
        vehicle_ids=traci.vehicle.getIDList()
        peatones_ids = [id for id in all_ids if id not in vehicle_ids]
        vehiculos_ids = [id for id in all_ids if id in vehicle_ids]
        for id in peatones_ids:
            personwaiting.append(traci.person.getWaitingTime(id))
        for id in vehiculos_ids:
            #traci.vehicle.getWaitingTime(id)
            vehiclewaiting.append(traci.vehicle.getAccumulatedWaitingTime(id))
        personwaiting_media.append(np.mean(personwaiting))
        vehiclewaiting_media.append(np.nanmean(vehiclewaiting))
        # take one step in the simulation
        traci.simulationStep()
        #sleep(0.1)
    traci.close()
    result_vehiclewaiting = np.nanmean(vehiclewaiting_media)
    print("Tiempo de espera acumulado medio de vehículos: ", result_vehiclewaiting)
    # Crear una gráfica
    plt.figure()
    # Añadir una línea horizontal roja discontinua en la posición de result_personwaiting
    plt.axhline(y=result_vehiclewaiting, color='r', linestyle='--')
    plt.plot(vehiclewaiting_media)
    plt.legend([f'Tiempo de espera acumulado medio de vehículos: {result_vehiclewaiting}'])
    plt.show()
    
    return [vehiclewaiting_media, result_vehiclewaiting]
